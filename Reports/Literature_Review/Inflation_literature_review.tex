...
\begin{equation}\label{eq: rho to a}
    \rho \propto a^{-3(1 + w)}.
\end{equation}
$\rho$ is the total energy density of the universe, though it can be used for the individual constituent of the universe, denoted by $\rho_i$, where the energy density of each constituent is determined by its barotropic parameter. Depending on what constituent is dominant in the universe, the relations vary as follows, with dark matter and baryonic matter grouped together: 
\vspace{-0.25cm}
\begin{table}[H]
    \centering
...
\end{table}
\vspace{-0.5cm}
If there are multiple constituents contributing significantly then the energy density is the sum of each constituent contribution and the barotropic parameter for each is 
...
\end{equation}
In order for accelerated expansion to occur, the condition $\epsilon \ll 1$ is required, and in the case of quasi-de Sitter inflation, $w\approx -1$, the potential energy of the field dominates the kinetic energy of the inflaton, 
...
$\alpha$-attractors are an example of a modern inflationary model which also align with the observational bounds. The two main models for $\alpha$-attractors are the T-model and the E-model \cite{kalloshPresentStatusInflationary2025} which take the form:
\begin{align}
...
There exists alternative propositions that attempt to solve the Horizon and Flatness Problems and to create the density perturbations observed. The main ones discussed here are String Gas Cosmology, Variable Speed of Light and the Ekpyrotic scenario though others exist such as the Matter Bounce hypothesis.  
...
This model assumes that the universe that the universe originated as an empty BPS \cite{Kan2024} state that lasted an exponentially long time prior to the Hot Big Bang \cite{Khoury2001}. This assumption is what explains the Flatness Problem by just assuming flatness. The Horizon Problem is solved as the colliding branes create global conditions for the universe. 

Problems with this model is that it is based off of speculative physics, and originally offered no reason for the attraction between the branes. Another problem is that the solution to the Horizon and Flatness Problems is setting the initial conditions such that they do not exist. However, consequentially this doesn't explain the fine-tuning problem, and instead makes it more prominent.   
