...
\begin{equation}\label{eq: rho to a}
    \rho \propto a^{-3(1 + w)}.
\end{equation}
$\rho$ is the total energy density of the universe, though it can be used for the individual constituent of the universe, denoted by $\rho_i$, where the energy density of each constituent is determined by its barotropic parameter. The properties of each constituent and its effect on the parameters the universe, the relations vary as shown in \cref{table: Composition relations}, where matter includes baryonic and dark matter.
...
If multiple constituents are contributing, then the energy density is the sum of each constituent contribution. Combining this with the Friedmann equation, \cref{eq: Friedmann Equation}, demonstrates the relation between the scale factor and time, 
...
As previously stated, \cref{eq: Temperature Relation to scale factor}, the temperature of the universe is inversely proportional to the scale factor, so as the universe expands, the temperature decreases, as the density of matter and energy decreases. Due to the speed of inflation, the temperature of the thermal bath would be drastically depleted, but at the end of inflation is when the Hot Big Bang occurs, which requires a significant thermal bath. Before explaining the mechanism of how this thermal bath is recovered, it is important to explain primordial density perturbations. 
...
Inflation in its simplest form is modelled by a single scalar field, $\phi$, called the inflaton. This field is minimally coupled to gravity and is parametrised an addition to the Einstein-Hilbert action 
...
\end{equation}
where $3H$ and $V'$ correspond to the frictional and potential term respectively. In the case of quasi-de Sitter inflation, $\ddot{\phi}$ is negligible, resulting in 
...
The slow-roll parameter, $\epsilon$, is the parametrisation change in the universe's expansion rate, 
